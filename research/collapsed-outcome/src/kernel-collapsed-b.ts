// KERNEL 3 — ONE word, and `R` carries the WORD.
//
// Same collapse as kernel 2 — no branches, no `ok` boolean, the carrier owns
// what "went well" means — with one difference: `ValueOf` does NOT unwrap. The
// word's TYPE survives into `R`, so the scope's own signature enumerates every
// refusal it can produce, and the word's intent name is the discriminant.
//
// The carrier needs no convention for that to hold: nothing has to be tagged by
// hand, because the word already carries its name in its type. The price moves
// to the CALLER, which unwraps before it can use the domain value.

const OUTCOME = Symbol.for('lntt.research.collapsed-b.outcome')
const WORD = Symbol.for('lntt.research.collapsed-b.word')
...
export const isWord = (x: unknown): x is Word<unknown, never> =>
  typeof x === 'object' && x !== null && WORD in x

// ONE branch, and what it holds is whatever the leaf returned — a domain value
// or a word, undisturbed.
export type Outcome<R> = { readonly [OUTCOME]: true } & { readonly result: R }

type AnyOutcome = Outcome<unknown>

// The word SURVIVES. This is the one line that separates this kernel from the
// one beside it.
export type ValueOf<R> = R extends AnyOutcome ? never : R
...
const outcomeOf = (r: unknown): Outcome<unknown> =>
  isOutcome(r) ? r : { [OUTCOME]: true, result: r }
