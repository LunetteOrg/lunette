...
export interface Scope {
  ...
  step<Self = this>(this: Self, s: Step): Self
  handle<Need extends object, R, Self = this>(
    this: Self,
    f: (app: Need, ctx: object) => R,
  ): Handler<NeedOf<Self> & Need, Awaited<R>>
}
...
function make(steps: readonly Step[]): Surface {
  return {
    handle(f: (app: object, ctx: object) => unknown) {
      const next = [...steps, leaf(f)]
      const run = (app: object) => runSteps(next, app)
      return Object.assign(run, { steps: next })
    },
    step(s: Step) {
      const next = [...steps, s]
