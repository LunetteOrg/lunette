...
| [`collapsed-outcome/`](./collapsed-outcome/) | what the scope's OUTCOME costs in four shapes — two branches (today), collapsed carrying the payload, collapsed carrying the word, and no outcome at all. Measured: collapsing is nearly free (~2 instantiations per scope) and its real price is that `ResultOf` stops being the domain type; a TRANSPARENT core is where the saving is (−36 per scope, nine runtime values down to one), it keeps the supply gate without a brand, and it charges the WRAP shape instead. Also that enumerating a scope's refusals needs none of them — it is one exported alias in every design | the scope outcome on #30; [the outcome leaving the core](../docs/decisions/outcome-leaves-core-scope-hands-back.md) cites it throughout | **FIRED** — the outcome left the core. Kept as the prior art it was decided on | #81 |
...
| ~~`no-scope-hosts/`~~ | the same wire chain on Hono, Express, tRPC and React Router with NO scope at all, handlers written the way an app author actually would — what each host needs, what is duplicated four times, which mistakes are SILENT, and which of scope's current pieces answer one of them | issue #76 (closed via PR #78); [a carrier needs no vocabulary at all](../docs/decisions/carrier-needs-no-vocabulary-at-all.md) | **RETIRED** — #76 closed and its findings are in `docs/decisions/carrier-needs-no-vocabulary-at-all.md`. Last state on `main` before removal: PR #78 | #83 (closed) |
