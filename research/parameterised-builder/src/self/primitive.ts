...
//   whether it terminates         `closes`, declared
//   what verbs it adds            `methods`, declared
//
// Three of the five ride positions the signature already has, so they are not
// declared twice and cannot drift from the code beside them. The two declared
// ones are declared because a runtime value cannot make a type-level claim:
// VERBS contribute to the builder's surface, and TERMINATION turns the builder
// into the concrete callable — see below for why that transition is needed.
//
...
// The things a step may say beyond its signature are ordinary VALUES, not
// phantoms, which is what makes them readable and writable:
//
//   { run, closes: true }                  it terminates
...
export interface StepValue<Run, M extends Verbs = Verbs> {
  readonly run: Run
  // PRESENT ⇒ this step TERMINATES, closing the builder into the callable. The
  // builder reads the KEY, not the value: assigning `{ run, closes: true }` to
  // a variable widens the property to `boolean`, and an absent `closes` also
  // resolves to `boolean` from its constraint, so by value the two are the same
  // type (measured). Typing it `true` here keeps `closes: false` from reading
  // as a way to say the opposite — omitting it is the way.
...
// WHY TERMINATION IS DECLARED AT ALL — and the reason is not the one it was
// introduced for. Dropping it was tried, on the argument that the fold already
// sees termination at RUNTIME (it does: a step that does not call `next` ends
// it) and that `R` could be read off the steps instead of at one closing point
// (it can: two of the three things a step returns contribute no value, and the
// brand tells them apart).
//
// Both of those hold. What does NOT hold is the CALL. A `this` parameter binds
// to the receiver of a METHOD call, and calling an object directly binds `this`
// to `void` — measured — so a call signature on the builder cannot read the
// accumulated `Self`, and `Need`, `Seed` and `R` are all invisible to it. The
// scope would be callable and untyped: no `DepGuard`, no typed parameters, no
// typed result. Intersecting a fresh concrete signature per step does not
// rescue it either, because two call signatures in an intersection become
// OVERLOADS and the stale one resolves first.
//
// So the transition from an intersection-accumulated builder to a CONCRETE
// type is what makes the call typed, and a declaration is what triggers it.
