...
**Status:** open design exploration, *not* a decision. Feeds GitHub issue
[#5 — Request scope in the dialects](https://github.com/LunetteOrg/lunette/issues/5).

Captured from a design session sparked by a real case: the `pelion/admin`
auth flow, where `requireAdmin(app, request)` is repeated across ~20 route
...
## Not HTTP-specific — a scope KIND

A processed event is another scope (`event-scoped`); a job, a message,
likewise. This unifies request-scope (issue #5) with the event-consumer
package (issue #10, `@lntt/listener`) under one mechanism: the same runtime,
a different **outcome codec** per kind — HTTP `return→200/4xx`, `throw→5xx`;
events `return→ack`, `throw→nack`.
