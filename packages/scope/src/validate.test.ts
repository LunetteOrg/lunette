...
describe("http validate('params', schema) — runtime coercion + the invalid branch", () => {
  it('coerces "42" → 42 on success and hands the leaf the typed params', async () => {
    ...
    const out = await h({}, { ...{ request: req }, params: { courseId: '42' } })
  ...
  it('a validation failure is the RETURNED `invalid` branch, never a throw', async () => {
    ...
    const out = await h({}, { ...{ request: req }, params: { courseId: 'abc' } })
  ...
  it('threads coerced params through guards; a guard abort short-circuits', async () => {
    ...
    const ok = await h({}, { ...{ request: req }, params: { courseId: '7' } })
    ...
    const denied = await h({}, { ...{ request: req }, params: { courseId: '0' } })
  ...
  it('validation runs BEFORE any guard — a bad param never reaches the stack', async () => {
    ...
    const out = await h({}, { ...{ request: req }, params: { courseId: 'not-a-number' } })
    expect(out.ok).toBe(false)
