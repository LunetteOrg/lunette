...
import { type Lunette, lunette } from '../../src/index.ts'
...
async function route0(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session0: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin0: c.adminRepo.byId(c.session0) + c.orderRepo.count(),
    }))
...
async function route1(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session1: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin1: c.adminRepo.byId(c.session1) + c.orderRepo.count(),
    }))
...
async function route2(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session2: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin2: c.adminRepo.byId(c.session2) + c.orderRepo.count(),
    }))
...
async function route3(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session3: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin3: c.adminRepo.byId(c.session3) + c.orderRepo.count(),
    }))
...
async function route4(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session4: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin4: c.adminRepo.byId(c.session4) + c.orderRepo.count(),
    }))
...
async function route5(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session5: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin5: c.adminRepo.byId(c.session5) + c.orderRepo.count(),
    }))
...
async function route6(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session6: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin6: c.adminRepo.byId(c.session6) + c.orderRepo.count(),
    }))
...
async function route7(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session7: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin7: c.adminRepo.byId(c.session7) + c.orderRepo.count(),
    }))
...
async function route8(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session8: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin8: c.adminRepo.byId(c.session8) + c.orderRepo.count(),
    }))
...
async function route9(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session9: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin9: c.adminRepo.byId(c.session9) + c.orderRepo.count(),
    }))
...
async function route10(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session10: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin10: c.adminRepo.byId(c.session10) + c.orderRepo.count(),
    }))
...
async function route11(app: App, inv: Invocation) {
  return lunette<App & Invocation>()
    .expose((c) => ({
      session11: c.sessionRepo.get(inv.scope.params.id ?? 'x'),
    }))
    .expose((c) => ({
      admin11: c.adminRepo.byId(c.session11) + c.orderRepo.count(),
    }))
