// LOAD-BEARING gate proof (design A — carrier capabilities). A fragment that
...
// A body-reading fragment with NO app deps, so DepGuard is trivially satisfied
// and only the CARRIER capability gate is under test.
const writeFrag = fragment()
  .body(z.object({ title: z.string() }))
  .handle((_deps: {}, ctx) => ({ echoed: ctx.body.title }))

// A param-only read fragment — requires no capability (Cap = never).
const readFrag = fragment()
  .input(z.object({ id: z.string() }))
  .handle((_deps: {}, ctx) => ({ id: ctx.params.id }))

// ── a body-capable host ACCEPTS the write fragment ──────────────────────────
const w = hono(chain, () => ({ env: { label: 'x' } }))
w.handler(writeFrag) // compiles: Hono's carrier provides 'body'
w.handler(readFrag)

// ── tRPC ACCEPTS reads but REJECTS the body fragment at the mount site ───────
type Ctx = App & { request: Request }
const t = initTRPC.context<Ctx>().create()
toProcedure(t.procedure, readFrag)
// @ts-expect-error host missing capability 'body' — a .body fragment cannot mount on tRPC
