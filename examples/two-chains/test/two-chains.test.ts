...
// that nothing leaks between them: not the app a route reads, not the seed,
// not the lifecycle.
const start = async () => {
...
describe('two chains on one Express app', () => {
  ...
  it('builds and disposes the two lifecycles independently', async () => {
    ...
    // the catalogue answering a request never touches the admin chain's state
    expect(admin.opened).toEqual(['audit'])

    await close()
    expect(catalog.closed).toEqual(['catalogue-db'])
    expect(admin.closed).toEqual(['audit'])
  })
