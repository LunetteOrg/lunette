...
const withAudit = layer<{ env: AdminEnv }, { entries: string[] }>(
  async (_ctx, next) => {
    opened.push('audit')
...
export const adminChain = lunette<{ env: AdminEnv }>()
  ...
  .expose('auth', (ctx) => ({
    accepts: (token: string | null) => token === ctx.env.TOKEN,
  }))
...
const findAuth = (
  ...
  deps.auth.accepts(req.header('authorization') ?? null)
    ? {}
...
const gated = scope(expressCarrier())
  .extend(guards)
  .guard(findAuth, (issues, { res }: { readonly res: Response }) =>
    res.status(401).json({ issues }),
  )

export const auditScope = gated.step(
  async (deps: { audit: { all(): string[] } }, { res }) =>
    res.json({ entries: deps.audit.all() }),
)

export const recordScope = gated.step(
  async (
    deps: { audit: { record(what: string): void; all(): string[] } },
    { res },
  ) => {
