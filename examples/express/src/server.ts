...
export function makeApp(env?: Env): Express {
  ...
  // needed. The shared `*Fragment` imports below are the multi-host portability
  // device; `feedFragment` still ships as their documented form.
  app.get('/feed', w.handler(fragment().guard(feedGuard).handle(feedHandler)))
