...
// error→status map); every `fragment()` wiring lives HERE, one declarative line
...
// import these fragments unchanged. A file `handlers.ts` and a sibling directory
...
// `.handle(feedHandler)`) instead of importing `feedFragment`.
...
const gated = () => fragment().guard(sessionGuard).guard(authGuard)
...
export const loginFragment = fragment().form(loginForm).guard(loginGuard).handle(() => ({
  ok: true as const,
}))
...
export const verifyFragment = fragment().body(verifyBody).guard(pendingGuard).handle(verifyHandler)

// ── auth: POST /logout ───────────────────────────────────────────────────────
export const logoutFragment = fragment().handle(logoutHandler)
